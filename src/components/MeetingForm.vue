...
const localFormData = ref<MeetingFormData>({ ...props.formData, coverFile: [] }); // Ensure coverFile is initialized as an empty array

watch(() => props.formData, (newVal) => {
  localFormData.value = { ...newVal, coverFile: [] }; // Reset coverFile when props.formData changes
});
...
onMounted(() => {
  if (localFormData.value) {
    const editor = new EWangEditor('#editor');
    editor.config.onchange = (html: string) => {
      localFormData.value!.meetingContent = html;
    };
    editor.create();
    editor.txt.html(localFormData.value.meetingContent || '');
  }
});
</script>
